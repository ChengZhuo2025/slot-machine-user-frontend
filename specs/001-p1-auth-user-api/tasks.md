...
- [ ] T110 [P1] [US1] 测试：验证可以向有效手机号发送短信验证码
- [ ] T111 [P1] [US1] 测试：验证使用正确验证码登录成功
- [ ] T112 [P1] [US1] 测试：验证使用错误验证码登录失败并显示错误消息
- [ ] T113 [P1] [US1] 测试：验证 60 秒倒计时防止快速重复发送
- [ ] T114 [P1] [US1] 测试：验证新用户获得 `is_new_user: true` 并看到欢迎消息
...
- [ ] T330 [P1] [US6] 测试：验证未登录用户访问个人中心时跳转到登录页
- [ ] T331 [P1] [US6] 测试：验证未登录用户访问购物车时跳转到登录页
- [ ] T332 [P1] [US6] 测试：验证未登录用户访问开锁页面时跳转到登录页
- [ ] T333 [P1] [US6] 测试：验证未登录用户访问扫码页面时跳转到登录页
- [ ] T334 [P1] [US6] 测试：验证登录成功后自动跳转回原目标页面
- [ ] T335 [P1] [US6] 测试：验证已登录用户可以正常访问受保护页面
- [ ] T336 [P1] [US6] 测试：验证公开页面（首页、商品详情）无需登录可正常访问
...
- [ ] T400 [P2] [US3] 更新 `src/services/request.js` - 集成 token 刷新逻辑，从 auth 服务导入 `refreshToken`
- [ ] T401 [P2] [US3] 更新 `src/services/request.js` - 刷新成功时，通过 `store.refreshTokens()` 更新 store 中的 tokens
- [ ] T402 [P2] [US3] 更新 `src/services/request.js` - 刷新失败时 (401)，调用 `store.logout()` 并重定向到登录页

### 登出实现

- [ ] T410 [P2] [US3] 更新登出组件/页面 - 在清除本地状态前调用 `authApi.logout()`
- [ ] T411 [P2] [US3] 更新 `src/stores/user.js` - 在 `logout()` action 中清除：`accessToken`、`refreshToken`、`tokenExpireTime`、`userInfo`
- [ ] T412 [P2] [US3] 更新登出处理器 - 为持久化 token 键值调用 `uni.removeStorageSync()`
- [ ] T413 [P2] [US3] 更新登出处理器 - 清理后跳转到 `/pages/user/login`

### 验证测试

- [ ] T420 [P2] [US3] 测试：验证 token 即将过期时（剩余 <5 分钟）自动刷新
- [ ] T421 [P2] [US3] 测试：验证刷新期间的并发请求都使用新 token 成功
- [ ] T422 [P2] [US3] 测试：验证过期的 refresh_token 导致重定向到登录页
- [ ] T423 [P2] [US3] 测试：验证登出清除所有凭证并重定向到登录页
...
- [ ] T500 [P2] [US4] 更新 `src/pages/user/index.vue` - 导入 user 服务，用 `userApi.getProfile()` 调用替换 mock 数据
- [ ] T501 [P2] [US4] 更新 `src/pages/user/index.vue` - 显示用户数据：头像、昵称、手机号（脱敏格式：138****1234）、会员等级、积分
- [ ] T502 [P2] [US4] 更新 `src/pages/user/index.vue` - 处理加载状态（获取个人信息时）
- [ ] T503 [P2] [US4] 更新 `src/pages/user/index.vue` - 处理错误状态，提供重试选项

### 编辑个人信息

- [ ] T510 [P2] [US4] 更新编辑个人信息组件/页面 - 表单提交时调用 `userApi.updateProfile(data)`
- [ ] T511 [P2] [US4] 更新编辑个人信息组件 - 处理昵称验证错误：显示"昵称包含违规内容"
- [ ] T512 [P2] [US4] 更新编辑个人信息组件 - 个人信息更新后显示成功提示
- [ ] T513 [P2] [US4] 更新编辑个人信息组件 - 成功更新后刷新个人信息数据

### 验证测试

- [ ] T520 [P2] [US4] 测试：验证个人信息页面显示来自 API 的正确用户信息
- [ ] T521 [P2] [US4] 测试：验证昵称更新成功保存
- [ ] T522 [P2] [US4] 测试：验证无效昵称显示适当的错误消息
- [ ] T523 [P2] [US4] 测试：验证手机号显示脱敏格式（138****1234）
...
- [ ] T600 [P3] [US5] 更新钱包页面 - 导入 user 服务，挂载时调用 `userApi.getWallet()`
- [ ] T601 [P3] [US5] 更新钱包页面 - 显示：余额、冻结余额、累计充值、累计消费
- [ ] T602 [P3] [US5] 更新钱包页面 - 格式化货币值，保留 2 位小数

### 交易记录

- [ ] T610 [P3] [US5] 更新交易记录页面/组件 - 实现 `loadTransactions(page, pageSize)` 函数
- [ ] T611 [P3] [US5] 更新交易记录页面 - 显示交易列表：类型名称、金额（+ 或 -）、创建时间
- [ ] T612 [P3] [US5] 更新交易记录页面 - 实现滚动加载更多：滚动到底部时增加页码
- [ ] T613 [P3] [US5] 更新交易记录页面 - 添加类型筛选下拉框：充值、消费、退款等
- [ ] T614 [P3] [US5] 更新交易记录页面 - 处理空状态：显示"暂无交易记录"
- [ ] T615 [P3] [US5] 更新交易记录页面 - 处理列表结束：显示"没有更多记录了"

### 验证测试

- [ ] T620 [P3] [US5] 测试：验证钱包余额正确显示
- [ ] T621 [P3] [US5] 测试：验证交易列表分页加载
- [ ] T622 [P3] [US5] 测试：验证类型筛选正常工作
- [ ] T623 [P3] [US5] 测试：验证空交易列表显示占位符
...
- [ ] T700 [P2] [FOUND] 创建 `src/utils/errorHandler.js` - 集中错误消息映射，提供中文用户友好消息
- [ ] T701 [P2] [FOUND] 更新 `src/services/request.js` - 添加网络错误处理：显示"网络连接失败，请检查网络设置"
- [ ] T702 [P2] [FOUND] 更新 `src/services/request.js` - 添加超时错误处理：显示"请求超时，请稍后重试"

### 加载状态

- [ ] T710 [P2] [FOUND] 检查所有调用 API 的页面 - 确保请求期间显示加载动画
- [ ] T711 [P2] [FOUND] 检查所有调用 API 的页面 - 请求期间禁用提交按钮，防止重复提交

### 清理

- [ ] T720 [P3] [FOUND] 更新 `src/mock/index.js` - 添加功能标志禁用 mock 数据：`const USE_MOCK = false`
- [ ] T721 [P3] [FOUND] 验证所有页面使用真实 API - 生产代码中无 mock 数据引用
